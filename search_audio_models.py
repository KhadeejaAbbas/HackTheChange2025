...
def search_audio_models():
    ...
    try:
        ...
        audio_models = []
        speech_keywords = ['whisper', 'speech', 'audio',
                           'voice', 'transcribe', 'tts', 'sound']

        for model in models:
            model_name = model.get('modelName', '').lower()
            model_id = model.get('modelId', '').lower()
            provider = model.get('providerName', '').lower()
            input_modalities = [m.lower()
                                for m in model.get('inputModalities', [])]
            output_modalities = [m.lower()
                                 for m in model.get('outputModalities', [])]
        ...
        if audio_models:
            print(
                f"🎵 Found {len(audio_models)} potentially audio-related models:")
            print("-" * 60)
            ...
            for i, model in enumerate(audio_models, 1):
                ...
                print(
                    f"   Status: {model.get('modelLifecycle', {}).get('status', 'Unknown')}")
                print("-" * 40)
        else:
